...
bank_account_t createBankAccount(uint32_t id, char *password, uint32_t balance)
{
    ...
    char temp[MAX_PASSWORD_LEN + SALT_LEN + 1];
...
int insertBankAccount(bank_account_t newAccount, uint32_t delay,uint32_t officeID)
{
    ...
    usleep(MS_TO_US(delay)); // TODO: nanosleep em vez de usleep
    ...
    if(pthread_mutex_unlock(&account_mutex[newAccount.account_id]) != 0) // TODO: garantir unlock
        return -2;
...
bool existsBankAccount(uint32_t id)
{
    ...
    if (accounts[id].account_id != ERROR_ACCOUNT_ID) { // TODO:  return valor
        return true;
    }
